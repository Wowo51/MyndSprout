...
{
    [TestClass]
    public sealed class DatabaseHardener_Deny_AlterAnyExternalLibrary_Isolation
    {
        [TestMethod]
        public async Task TestMethod()
        {
            ...
            var dbName = $"SqlContain_Isolation_AlterAnyExternalLibrary_{Guid.NewGuid():N}";
            ...
            try
            {
                ...
                try
                {
                    ...
                    try
                    {
                        await SqlHelpers.ExecBatchesAsync(dbConn, "DENY ALTER ANY EXTERNAL LIBRARY TO public;");
                        Assert.IsTrue(true);
                    }
                    catch (Exception ex)
                    {
