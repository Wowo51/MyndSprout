...
{
    [TestClass]
    public sealed class DatabaseHardener_Deny_AlterAnyExternalDataSource_Isolation
    {
        [TestMethod]
        public async Task TestMethod()
        {
            ...
            var dbName = $"SqlContain_Isolation_AlterAnyExternalDataSource_{Guid.NewGuid():N}";
            ...
            try
            {
                ...
                try
                {
                    ...
                    try
                    {
                        await SqlHelpers.ExecBatchesAsync(dbConn, "DENY ALTER ANY EXTERNAL DATA SOURCE TO public;");
                        Assert.IsTrue(true);
                    }
                    catch (Exception ex)
                    {
