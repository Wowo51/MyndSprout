...
{
    [TestClass]
    public sealed class DatabaseHardener_AllowSkippedDeny_Propagation
    {
        [TestMethod]
        public async Task OptionsAllowSkippedDeny_IsPropagatedInAggregateException()
        {
            ...
            try
            {
                ...
                Assert.IsTrue(aggEx.Data.Contains("Options.AllowSkippedDeny"), "AggregateException must include Options.AllowSkippedDeny");
                object? rawAllow = aggEx.Data.Contains("Options.AllowSkippedDeny") ? aggEx.Data["Options.AllowSkippedDeny"] : null;
                Assert.AreEqual(false, rawAllow is bool ? (bool)rawAllow : false, "Options.AllowSkippedDeny must be false as passed in options.");
